document.querySelectorAll('.services-item-card').forEach(card => {
  ...
  const questions = [
      { text: "", options: ["", "", ""] },
      { text: "Какие услуги фулфилмента вам нужны?", options: ["Хранение", "Проверка на брак", "Упаковка", "Маркировка", "Доставка", "Возврат брака", "Контент", "Комплекс под ключ"] },
      { text: "На какие маркетплейсы планируется поставка товаров?", options: ["Wildberries (Вайлдберриз)", "Ozon (Озон)", "Другой"] },
  ...
  function submitForm() {
      ...
      document.getElementById('final-message').innerHTML = `Спасибо, ${name}! Ваши данные успешно отправлены. Пока мы с вами свяжемся, вы можете ознакомится с ответами на часто задоваемые вопросы или <a href="tel:+1234567890">позвонить нам</a>.`;
  }
